<?php
 ...
 $addColorGet = array( 'page' => 'calculator_aj_kolor_individual_subcat_menu', 'do' => 'add');
 // Tytuł strony
 $khc_title_page = "Farby špeciálne";
 if (isset($_GET['do'])) {
     $where = $_GET['do'];
     if($where =='edit'){
 ...
 <?php if(!isset($_GET['do'])): ?><a class="btn btn-success mb-3" href="<?php echo add_query_arg( $addColorGet, admin_url( 'admin.php' ) );?>">Pridať novú farbu</a><?php endif;?>
 ...
 if (isset($_GET['do'])) {
     ...
     switch ($where) {
         case 'edit':
             return khc_KolorIndividualEdit($khcColorId);
             break;
         case 'add':
             return khc_KolorIndividualAdd();
             break;
         case 'remove':
     ...
     return khc_showAllColorIndividual();
 }
