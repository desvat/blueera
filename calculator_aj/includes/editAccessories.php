...
if(isset($_POST['submit'])){
    ...
    if(isset($_FILES['fileToUpload']['tmp_name']) && $_FILES['fileToUpload']['tmp_name'] != ""){
        ...
        $getImage = new kalkulatorHcMaterial();
            $oldImage =$getImage->getAccessories($id);
        ...
        $accessories->editAccessories($edytowaneParamenty,$gdzieParamenty);
        $komunikat = '<p style="color:green;">Úspešne aktualizované!</p>';
    }else{
      
        $komunikat = "";
    }
    $selectedAccessories = $accessories->getAccessories($id);

    $queryPowrotKolory = array( 'page' => 'calculator_aj_accessories_subcat_menu');

    $remove_args = array( 'page' => 'calculator_aj_accessories_subcat_menu', 'do' => 'remove','id'=>$id ); 
...
?>
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="mb-3">
        <?php 
                if(file_exists(KHC_PATH.'uploads/'.$selectedAccessories->image) && $selectedAccessories->image != ""):?>
                 <div class="col-md-4">
                     <div class="row mb-3">
                        <img src="<?php echo KHC_PATH.'uploads/'.$selectedAccessories->image; ?>" class="img-fluid" alt="">
                     </div>
        ...
        <input type="text" class="form-control" name="tytul" id="TytulKolor" aria-describedby="emailHelp" value="<?php echo $selectedAccessories->tytul;?>" required>
