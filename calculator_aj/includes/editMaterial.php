...
if(isset($_POST['submit'])){
    ...
    if(isset($_FILES['fileToUpload']['tmp_name']) && $_FILES['fileToUpload']['tmp_name'] != ""){
        ...
        $getImage = new kalkulatorHcMaterial();
            $oldImage =$getImage->getMaterial($id);
        ...
        $Material->editMaterial($edytowaneParamenty,$gdzieParamenty);
        $komunikat = '<p style="color:green;">Zaktualizowano pomyślnie!</p>';
    }else{
      
        $komunikat = "";
    }
    $selectedMaterial = $Material->getMaterial($id);

    $queryPowrotMaterialy = array( 'page' => 'calculator_aj_material_subcat_menu');

    $remove_args = array( 'page' => 'calculator_aj_material_subcat_menu', 'do' => 'remove','id'=>$id ); 
...
?>
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="mb-3">
        <?php 
                if(file_exists(KHC_PATH.'uploads/'.$selectedMaterial->image) && $selectedMaterial->image != ""):?>
                 <div class="col-md-4">
                     <div class="row mb-3">
                        <img src="<?php echo KHC_PATH.'uploads/'.$selectedMaterial->image; ?>" class="img-fluid" alt="">
                     </div>
        ...
        <input type="text" class="form-control" name="tytul" id="TytulKolor" aria-describedby="emailHelp" value="<?php echo $selectedMaterial->tytul;?>" required>
        <div id="emailHelp" class="form-text">Zadajte názov materiálu, ktorý sa má zobraziť</div>
    ...
    <a class="khc_powrot" href="<?php echo add_query_arg( $queryPowrotMaterialy, admin_url( 'admin.php' ) );?>">< späť</a>
...
<p>Czy na pewno chcesz usunąć ten materiał?</p>
